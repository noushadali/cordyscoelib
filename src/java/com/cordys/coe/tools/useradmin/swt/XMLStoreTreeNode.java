...
public class XMLStoreTreeNode extends AbstractCordysTreeNode
{
     ...
     */
    public XMLStoreTreeNode(Tree tParent, int iStyle, XMLStoreItem xsiItem, ICordysConnection ccConnection)
    {
     ...
     * @param ccConnection The LDAP connection.
     */
    public XMLStoreTreeNode(TreeItem tiParent, int iStyle, XMLStoreItem xsiItem, ICordysConnection ccConnection)
    {
    ...
    public void loadChildren() throws Exception
    {
        ...
        if (cConnector != null)
        {
            ...
            Node iMethod = cConnector.createSoapMethod(null, null, "GetCollection", "http://schemas.cordys.com/1.0/xmlstore",
                    sUser);
            Node nEnvelope = cConnector.getEnvelope(iMethod);

            Element iFolder = (Element) XMLHelper.createTextElement("folder", xsiTemp.getKey(), iMethod);
    ...
    /**
     * This method initializes the tree item. Based on the LDAP entry the type is determined and initialized accordingly.
     */
    @Override
    ...
    /**
     * This method returns the user to get the collection with. If the level is ISV then trhe user SYSTEM is used. Otherwise the
     * first organizational user is used.
     * 
