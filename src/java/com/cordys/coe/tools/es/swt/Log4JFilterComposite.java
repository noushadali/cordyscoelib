...
public class Log4JFilterComposite extends Composite
{
     ...
     */
    private IEventServiceClient m_escClient;
    /**
    ...
    /**
     * Holds the currently entered regex for the filter.
     */
    private String m_sRegEx;
    /**
     * Holds the compiled pattern for the regex.
     */
     ...
     */
    public Log4JFilterComposite(Composite parent, int iStyle, IEventServiceClient escClient)
    {
        super(parent, iStyle);
        ...
        final Text m_tRegEx = new Text(gGeneral, SWT.BORDER);
        m_tRegEx.setLayoutData(new GridData(GridData.FILL, GridData.CENTER, true, false, 2, 1));
        m_tRegEx.addModifyListener(new ModifyListener()
            {
                public void modifyText(ModifyEvent arg0)
                {
                    m_sRegEx = m_tRegEx.getText();

                    try
                    {
                        m_pPattern = Pattern.compile(m_sRegEx, Pattern.DOTALL);
                    }
                    catch (Exception e)
                    {
                        m_pPattern = null;
    ...
    public boolean shouldDisplay(Log4JLogEvent lleEvent)
    {
        ...
        if (bReturn == true)
        {
            ...
            if ((bReturn == true) && ((m_sRegEx != null) && (m_sRegEx.length() > 0)))
            {
                // Check the regex
                String sMessage = lleEvent.getMessage();

                if ((sMessage != null) && (sMessage.length() > 0))
                {
                    bReturn = m_pPattern.matcher(sMessage).matches();
                }
            }
        }
