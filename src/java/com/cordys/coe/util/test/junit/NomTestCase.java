...
/**
 * Helper base class for JUnit tests that require NOM nodes.
 * Helps with keeping track of the allocated nodes.
 * This class also automatically checks for memory leaks
 * and enables NOM exceptions during testing.
 *
 * @author mpoyhone
 */
public class NomTestCase extends TestCase
{
    ...
    /**
     * Contains references to all created NOM nodes. Nodes
     * created with the 'noGarbage' methods are not added to this list.
     */
    private List<Integer> lGarbage = new ArrayList<Integer>(50);
    /**
     * If <code>true</code> memory leaks are also check if the processing
     * ended in error.
     */
    ...
    /**
     * Initializes the test case. This sets the NOM leak info baseline
     * and enables NOM exceptions.
     */
    ...
    /**
     * Deletes all garbage NOM nodes and assets that there were no 
     * NOM leaks. This also disables the NOM exceptions.
     */
    public void uninitialize()
    {
        ...
        Document.setXMLExceptionsEnable(false);
    }
    ...
    /**
     * Loads the XML from a file into NOM XML. 
     * The root node is added to the garbage list.
     * @param fFile File object pointing to the file to be loaded.
     * @return XML root node.
     */
    public int loadXmlFile(File fFile) throws Exception
    {
    ...
    /**
     * Loads the XML resource (a file in the classpath) into NOM XML. 
     * The root node is added to the garbage list.
     * @param sResourcePath Resource path. If relative (does not start 
     *          with '/' it is relative to the parent package).
     * @return XML root node.
     */
    ...
    /**
     * Loads the XML resource (a file in the classpath) into NOM XML. 
     * The XML is not added to the garbage list.
     * @param sResourcePath Resource path. If relative (does not start 
     *          with '/' it is relative to the parent package).
     * @return XML root node.
     */
    ...
    /**
     * Tests that the two NOM nodes are equal. Currently this uses a simple
     * writeToString() check.
    ...
    {
        String sInfo = Document.getLeakInfo(0);
    ...
    /**
     * Sets the checkForLeaksOnError. If false and the success flag
     * is also false, memory leaks are not checked. 
     *
    ...
    /**
     * Sets the success flag value. <code>true</code> means that the
     * test run passed. 
     *
