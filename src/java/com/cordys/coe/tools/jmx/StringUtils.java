...
public class StringUtils
{
    ...
    /*
     * this function is be called recursively to display multi-dimensional arrays, e.g. int[][]
     */
    /**
     * DOCUMENTME.
     *
    ...
    public static String toString(String type, boolean detailed)
    {
        ...
        try
        {
            ...
            if (clazz.isArray())
            {
                if (detailed)
                {
                    return toString(clazz.getComponentType().getName(), detailed) + "[]"; // $NON-NLS-1$
                }
                else
                {
                    return toString(clazz.getComponentType().getSimpleName(), detailed) + "[]"; // $NON-NLS-1$
                }
            }
        }
        catch (ClassNotFoundException e)
        {
            ...
            { // $NON-NLS-1$ //$NON-NLS-2$
                return toString(type.substring(2, type.length() - 1), detailed) + "[]"; // $NON-NLS-1$
            }
        }
        return type;
    }
     ...
     * @return  DOCUMENTME
     */
    private static String toDetailedString(Object arrayObj)
    {
     ...
     * @return  DOCUMENTME
     */
    private static String toSimpleString(Object arrayObj)
    {
        Assert.isNotNull(arrayObj);
        Assert.isLegal(arrayObj.getClass().isArray());
