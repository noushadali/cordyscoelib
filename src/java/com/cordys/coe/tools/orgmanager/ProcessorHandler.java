...
public class ProcessorHandler
{
    ...
    /** Holds all organizations that are available */
    private LinkedHashMap<String, LDAPEntry> m_organizations = new LinkedHashMap<String, LDAPEntry>();
                                                      ...
                                                      throws CordysGatewayClientException
    {
        ...
        for (Entry<String, LDAPEntry> e : m_organizations.entrySet())
        {
            alReturn.add(new ObjectData<LDAPEntry>(LDAPUtils.getAttrValue(e.getValue(), "description"), e.getValue()));
        }
                     ...
                     throws CordysGatewayClientException, CordysSOAPException, TransformerException
    {
        ...
        // First initialize the map with all organizations.
        for (LDAPEntry le : aeOrgs)
        {
            ...
            m_organizations.put(le.getDN(), le);
        }

        // Add all service containers
        LDAPEntry[] aleSPs = m_cgcClient.searchLDAP(m_cgcClient.getSearchRoot(), LDAPConnection.SCOPE_SUB,
                                                    "(objectclass=bussoapprocessor)");

        for (LDAPEntry le : aleSPs)
        {
            Processor p = new Processor(le);

            LinkedHashMap<String, Processor> lhmProcessors = m_lhmOrgsAndProcessors.get(p.getOrganization());
            lhmProcessors.put(p.getDN(), p);

            // If the processor is a monitor we need to add it to the list.
            if (p.isMonitor())
            {
                m_lhmMonitorProcessors.put(p.getComputer(), p.getDN());
            }
        }
                       ...
                       throws CordysGatewayClientException, CordysSOAPException, TransformerException
    {
        ...
        for (String sDN : m_lhmMonitorProcessors.values())
        {
            ...
            try
            {
                ...
                // Find all processors.
                NodeList nlProcesses = XPathHelper.selectNodeList(eResponse, "//mon:tuple/mon:old/mon:workerprocess",
                                                                  m_epr);
                ...
                for (int iCount = 0; iCount < nlProcesses.getLength(); iCount++)
                {
                    ...
                    pProcessor.setProcessID(XPathHelper.getIntegerValue(nWorkerProcess, "mon:process-id/text()", m_epr,
                                                                        -1));
                    pProcessor.setStatus(XPathHelper.getStringValue(nWorkerProcess, "mon:status/text()", m_epr,
                                                                    "Stopped"));
                    pProcessor.setTotalNomMemory(XPathHelper.getLongValue(nWorkerProcess, "mon:totalNOMMemory/text()",
                                                                          m_epr, -1));
                    pProcessor.setTotalNOMNodes(XPathHelper.getLongValue(nWorkerProcess,
                                                                         "mon:totalNOMNodesMemory/text()", m_epr, -1));
                    pProcessor.setTotalCpuTime(XPathHelper.getStringValue(nWorkerProcess, "mon:totalCpuTime/text()",
                                                                          m_epr, "00:00:00"));
                    pProcessor.setVirtualMemoryUsage(XPathHelper.getLongValue(nWorkerProcess,
                                                                              "mon:virtualMemoryUsage/text()", m_epr,
                                                                              -1));
                    pProcessor.setResidentMemoryUsage(XPathHelper.getLongValue(nWorkerProcess,
                                                                               "mon:residentMemoryUsage/text()", m_epr,
                                                                               -1));
                    ...
                    pProcessor.setTotalSOAPDocumentsProcessed(XPathHelper.getLongValue(nWorkerProcess,
                                                                                       "mon:busdocs/text()", m_epr,
                                                                                       -1));
                    pProcessor.setTotalProcessingTime(XPathHelper.getLongValue(nWorkerProcess,
                                                                               "mon:processing-time/text()", m_epr,
                                                                               -1));
                    pProcessor.setLastProcessingTime(XPathHelper.getLongValue(nWorkerProcess, "mon:last-time/text()",
                                                                              m_epr, -1));
     ...
     * @param   sOrganization   The organization to stop/start. If left null the action will be done for all
     *                          organizations.
     * @param   bIncludeManual  Whether or not to include the non-automatics in the action list.
     *
